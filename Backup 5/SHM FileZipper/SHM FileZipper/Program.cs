...
namespace SHM_FileZipper
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            byte[] h = File.ReadAllBytes("Batman Works Alone.FLV");
            //byte[] h = { 255 };
            byte[] l = { 5, 9, 7, 6 };
            byte[] lol = { 0X6A, 0X43, 0XF6, 0X91, 0X52, 0XEA, 0X88, 0X09 };
            sz = new SHMZipper(h);
            //Console.Write(sz.FileMatrix.StringRepresentation + "\n");
            //sz.initializeWorkingMatrix(true, true, false, false, 3, 7);
            sz.processWorkingMatrix(true, false, false, false, 0, 0);
            //Console.Write(sz.workingMatrix.StringRepresentation + "\n");
            //foreach (uint k in sz.privateDiagNoOfOnesToBeFilled)
            //    Console.WriteLine(k);
            Console.WriteLine();
            //foreach (uint k in sz.privateRevDiagNoOfOnesToBeFilled)
            //    Console.WriteLine(k);
