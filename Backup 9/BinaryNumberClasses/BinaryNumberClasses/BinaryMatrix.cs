    ...
    public class BinaryMatrix : BinaryStaticClass, ICloneable
    {
        ...
        public byte[] ByteRepresentation
        {
            ...
            {   return this.RawBinaryData.ByteRepresentation;
                //return getByteRepresentationFromMatrix();
            }
            set
        ...
        public byte[] getByteRepresentationFromMatrix()
        {
            byte[] outByteStream = new byte[privateSplitInputBinaryData.Length * privateSplitInputBinaryData[0].ByteRepresentation.Length];
            uint k = 0;
            foreach (BinaryNumber bn in privateSplitInputBinaryData)
            {
                foreach (byte b in bn.ByteRepresentation)
                {
                    outByteStream[k++] = b;
                }
            }
